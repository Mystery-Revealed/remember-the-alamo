...
// End Session (deletes everything from memory). Remember the Alamo is single-role
// solo, so students start on their own as soon as they're approved — no pairing needed.
...
export default function CommandCenter() {
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Remember the Alamo! · everyone marches with Houston's army · TEKS 7.3A, 7.3B, 7.3C, 7.1B</p>
  ...
  const cls = roster?.classAccuracy?.army || { count: 0, average: 0 };
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Remember the Alamo! · session-only data · download the PDF before you end</p>
        </div>
      ...
      <footer className="app-footer">7th Grade Texas History · Remember the Alamo! · TEKS 7.3A, 7.3B, 7.3C, 7.1B</footer>
    </div>
  );
}
...
function downloadPdf(roster) {
  ...
  const doc = new jsPDF();
  const BRAND = [44, 74, 99]; // campaign indigo

  doc.setFontSize(16);
  doc.text('Remember the Alamo! — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  doc.text('7th Grade Texas History · Remember the Alamo! · TEKS 7.3A, 7.3B, 7.3C, 7.1B', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`remember-the-alamo_${roster.joinCode}_${date}.pdf`);
}
